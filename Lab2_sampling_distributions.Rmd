...
```{r, echo=TRUE}
scoring_succes_1 <- rbinom(1, size = 10, prob = 0.4)
```

If we would like to calculate probability of succes, here you go!
```{r, echo=TRUE}
prob_1 <- scoring_succes_1 / 10
```

Let's now assume that basketball player is doing this exercises 10 times per day and the basketball player is having training 5 days per week. All together, we would expect this person to repeat such exercise 50 times per week, 200 times per month and 2600 times per year

Now lets calculate number of sucesses and probabilities per each of those situations

Daily number of successes will be;

```{r, echo=TRUE}
scoring_succes_10 <- rbinom(10, size = 10, prob = 0.4)
```


Daily probability of having successes will be;

```{r, echo=TRUE}
prob_10 <- scoring_succes_10 / 10
```


Weekly number of successes will be;

```{r, echo=TRUE}
scoring_succes_50 <- rbinom(50, size = 10, prob = 0.4)
```


Weekly probability of having successes will be;

```{r,echo=TRUE}
prob_50 <- scoring_succes_50 / 10
```

Monthly number of successes will be;

```{r, echo=TRUE}
scoring_succes_200 <- rbinom(200, size = 10, prob = 0.4)
```


Monthly probability of having successes will be;

```{r, echo=TRUE}
prob_200 <- scoring_succes_200 / 10
```

Yearly number of successes will be;

```{r, echo=TRUE}
scoring_succes_2600 <- rbinom(2600, size = 10, prob = 0.4)
```


Yearly probability of having successes will be;

```{r, echo=TRUE}
prob_2600 <- scoring_succes_2600 / 10
```


Thanks to par(mfrow = c(4,1)) command, we could set upgraph window to have 4 rows and 1 column.

```{r, echo=TRUE}
par(mfrow = c(4,1))
hist(prob_10, breaks = 5, xlab = "Sample proportion")
hist(prob_50, breaks = 10, xlab = "Sample proportion")
hist(prob_200, breaks = 10, xlab = "Sample proportion")
hist(prob_2600, breaks = 10, xlab = "Sample proportion")
```
